...
const bootstrapPlanForSeller = async (sellerId) => {
  try {
    ...
    try {
      ...
      // Error updating sync tracking for automatic free plan assignment suppressed
    }
    ...
    // Error bootstrapping plan for seller suppressed
    return [];
  }
};
...
const getPlanUsageSummary = async (sellerId) => {
  ...
  // Calculate summary:
  // - Limits only from active (non-expired) plans
  // - Usage from ALL paid plan orders (including expired ones)
  ...
  // 2. Process Usage (All paid plan orders)
  planEntries.forEach(({ order, plan }) => {
    ensurePlanLimitsApplied(order, plan);
