...
const verifySeller = async (req, res, next) => {
  try {
    //('verifySeller middleware - headers:', req.headers);
    //('verifySeller middleware - x-seller-id:', req.headers['x-seller-id']);
    ('verifySeller middleware - all sellerId sources:', {
    ...
    if (mongoState !== 1) { // 1 = connected
      ...
      //('Offline mode - extracted sellerId:', sellerId);
      if (sellerId) {
    ...
    const sellerId = req.body.sellerId || req.query.sellerId || req.headers['x-seller-id'];
    //('Normal mode - extracted sellerId:', sellerId);
    //('Normal mode - sellerId truthy check:', !!sellerId);
