    ...
    changedDataTypes.forEach(dataType => {
      switch (dataType) {
        ...
        case 'products':
          ...
          fetchPromises.push((async () => {
            try {
              ...
              // console.log(`🔄 GET ALL DATA: Total batches in DB: ${allBatches.length}`);
        ...
        case 'plans':
          ...
          break;
        case 'planOrders':
          // Only return valid (non-expired) plan orders
...
const getPlanOrders = async (req, res) => {
  try {
    ...
    const PlanOrder = require('../models/PlanOrder');
    const planOrders = await PlanOrder.find({ sellerId: sellerId, status: 'active' }).sort({ createdAt: -1 });
