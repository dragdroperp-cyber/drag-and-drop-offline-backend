  ...
  try {
    ...
    try {
      ...
      if (saveError.code === 11000 && saveError.keyPattern && saveError.keyPattern.localId) {
        console.warn(`[CREATE_CUSTOMER] Race condition detected for localId: ${req.body.localId}. Returning existing customer.`);
...
const updateCustomer = async (req, res) => {
  try {
    ...
    if (updates.mobileNumber && updates.mobileNumber !== customer.mobileNumber) {
      const existingCustomer = await Customer.findOne({
        sellerId,
...
const createProduct = async (req, res) => {
  try {
    const sellerId = req.sellerId;
    const { name, barcode, categoryId, unit, lowStockLevel, description, localId } = req.body;
    ...
    try {
      ...
      // Handle duplicate localId race condition
      if (saveError.code === 11000 && saveError.keyPattern && saveError.keyPattern.localId) {
        console.warn(`[CREATE_PRODUCT] Race condition detected for localId: ${localId}. Returning existing product.`);
...
const updateProduct = async (req, res) => {
  try {
    ...
    const allowedUpdates = ['name', 'barcode', 'categoryId', 'unit', 'lowStockLevel', 'description', 'isActive', 'trackExpiry'];
    allowedUpdates.forEach(field => {
      if (updates[field] !== undefined) {
...
const createProductBatch = async (req, res) => {
  try {
    ...
    try {
      ...
      // console.log(`🔄 Sync tracking updated: productBatches for seller ${sellerId}`);
    } catch (trackingError) {
...
const getAllData = async (req, res) => {
  try {
    ...
    const dataTypesToCheck = ['customers', 'products', 'productBatches', 'orders', 'transactions', 'purchaseOrders', 'categories', 'refunds', 'plans', 'planOrders', 'expenses'];
    ...
    if (lastFetchTimes && typeof lastFetchTimes === 'object') {
      // console.log('🔄 GET ALL DATA: Checking for data changes since last fetch times');
      // console.log('🔄 GET ALL DATA: Current server time:', new Date().toISOString());

      // Get sync tracking to check for latest update times
      let syncTracking = null;
      try {
        syncTracking = await SyncTracking.findOne({ sellerId });
        // console.log('🔄 GET ALL DATA: Retrieved sync tracking:', syncTracking ? 'found' : 'not found');
      } catch (trackingError) {
...
const upgradePlan = async (req, res) => {
  try {
    ...
    // console.log(`🔄 Plan upgrade: Activating plan "${plan.name}" for seller ${seller.name} (free plan or existing plan switch)`);
...
const createRazorpayOrder = async (req, res) => {
  try {
    ...
    // Create pending PlanOrder record for tracking
    const planOrder = new PlanOrder({
      sellerId: seller._id,
      planId: plan._id,
      expiryDate: new Date(Date.now() + getPlanDurationMs(plan)), // Required field
      durationDays: plan.durationDays || 30,
      price: plan.price,
      status: 'paused',
      paymentStatus: 'pending',
      razorpayOrderId: razorpayOrder.id,
      customerLimit: plan.maxCustomers ?? null,
    ...
    res.json({
      ...
      data: {
        ...
        planId: plan._id.toString(),
        planOrderId: planOrder._id.toString(), // Pass our internal order ID too
        planName: plan.name,
...
const verifyRazorpayPayment = async (req, res) => {
  try {
    ...
    if (planOrder) {
      ...
      if (planOrder.razorpayOrderId !== rOrderId) {
        return res.status(400).json({
          success: false,
      ...
      planOrder.razorpayPaymentId = rPaymentId;
      planOrder.razorpaySignature = rSignature;
