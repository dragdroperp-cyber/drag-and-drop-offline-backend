...
const getAllData = async (req, res) => {
  try {
    ...
    const [customers, products, orders, transactions, vendorOrders, categories, refunds] = await Promise.all([
      Customer.find({ sellerId }).sort({ createdAt: -1 }),
      Product.find({ sellerId }).populate('categoryId', 'name').sort({ createdAt: -1 }),
      Order.find({ sellerId }).populate('customerId', 'name mobileNumber').lean().sort({ createdAt: -1 }),
      ...
      Refund.find({ sellerId }).sort({ createdAt: -1 })
    ]);
...
const verifyRazorpayPayment = async (req, res) => {
  try {
    ...
    if (planOrder) {
      ...
      // For mini plans, keep status as 'paused' - don't activate (they are top-ups)
      // For other plans, activate immediately
      const isMiniPlan = planDoc?.planType === 'mini';
      if (isMiniPlan) {
        planOrder.status = 'paused';
        planOrder.lastActivatedAt = null; // Don't set activation time for mini plans
      } else {
      ...
      // For mini plans, create order with 'paused' status (don't activate)
      ...
      planOrder = new PlanOrder({
        ...
        razorpaySignature,
        paymentStatus: 'completed',
        status: isMiniPlan ? 'paused' : 'active',
        lastActivatedAt: isMiniPlan ? null : now,
        accumulatedUsedMs: 0,
    ...
    // Don't update currentPlanId for mini plans (they are top-ups, not main plans)
    // Mini plans can be purchased multiple times and should not replace the seller's current plan
