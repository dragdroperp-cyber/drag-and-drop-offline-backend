...
const setActivePlanForSeller = async ({ sellerId, planId, planOrderId, allowCreateOnMissing = true }) => {
  try {
    ...
    // Update currentPlanId for non-mini plans only
    if (planType !== 'mini') {
...
const switchToValidPlan = async (req, res) => {
  try {
    ...
    console.log(`🔄 switchToValidPlan called for seller: ${sellerId}`);

    if (!sellerId) {
      console.log('❌ switchToValidPlan: Seller ID is required');
    ...
    const seller = await loadSeller(sellerId);
    if (!seller) {
      console.log('❌ switchToValidPlan: Seller not found');
    ...
    console.log(`🔄 switchToValidPlan: Seller found: ${seller.name}, currentPlanId: ${seller.currentPlanId}`);

    const planOrders = await loadSellerPlanOrders(sellerId);
    console.log(`🔄 switchToValidPlan: Found ${planOrders.length} plan orders`);
    const now = new Date();
    ...
    if (seller.currentPlanId) {
      ...
      if (currentPlanOrder) {
        ...
        console.log(`🔄 switchToValidPlan: Current plan ${currentPlanOrder._id} - status: ${currentPlanOrder.status}, remainingMs: ${remainingMs}`);
        if (remainingMs > 0 && currentPlanOrder.status !== 'expired') {
          currentPlanExpired = false;
        }
      } else {
        console.log(`🔄 switchToValidPlan: Current plan order ${seller.currentPlanId} not found in planOrders`);
      }
    } else {
      console.log(`🔄 switchToValidPlan: No currentPlanId set for seller`);
    }

    console.log(`🔄 switchToValidPlan: Current plan expired: ${currentPlanExpired}`);

    if (!currentPlanExpired) {
      console.log(`✅ switchToValidPlan: Current plan is still valid, no switching needed`);
    ...
    const validPlanOrders = planOrders.filter(order => {
      ...
      if (seller.currentPlanId && order._id.equals(seller.currentPlanId)) {
        console.log(`⏭️ switchToValidPlan: Skipping current expired plan ${order._id}`);
        return false;
      }

      const remainingMs = computeRemainingMs(order, order.planId, now);
      const isValid = remainingMs > 0 && order.status !== 'expired';
      console.log(`🔄 switchToValidPlan: Plan ${order._id} (${order.planId?.name || 'Unknown'}) - status: ${order.status}, remainingMs: ${remainingMs}, valid: ${isValid}`);
      return isValid;
    });

    console.log(`🔄 switchToValidPlan: Found ${validPlanOrders.length} valid alternative plans`);

    if (validPlanOrders.length === 0) {
      console.log(`❌ switchToValidPlan: No valid alternative plans found`);
      return res.status(404).json({
        success: false,
    ...
    console.log(`🔄 Switching expired current plan to valid alternative: ${bestAlternative.planId.name} for seller ${seller.name}`);
    ...
    console.log(`✅ Successfully switched to alternative plan: ${bestAlternative.planId.name} for seller ${seller.name}`);
    ...
    console.error('Switch to valid plan error:', error);
...
const reactivateCurrentPlan = async (req, res) => {
  try {
    ...
    console.log(`🔄 Reactivating current plan: ${planDoc.name} for seller ${seller.name} (${seller.email})`);
    ...
    refreshExpiryDate(currentPlanOrder, planDoc, remainingMs, now);
    ...
    try {
      ...
      console.error('Error updating sync tracking for plan reactivation:', trackingError);
    }

    console.log(`✅ Current plan reactivated: ${planDoc.name} is now active for seller ${seller.name}`);
    ...
    console.error('Reactivate current plan error:', error);
    return res.status(500).json({
      success: false,
      message: 'Internal server error',
      error: error.message,
