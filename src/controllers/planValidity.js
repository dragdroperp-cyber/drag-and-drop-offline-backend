  ...
  try {
    ...
    if (isMiniPlan && targetPlanOrder.paymentStatus && targetPlanOrder.paymentStatus.toLowerCase() !== 'completed') {
      ...
      try {
        ...
        //(`🔄 Sync tracking updated: planOrders for new mini plan order ${targetPlanOrder._id}`);
      } catch (trackingError) {
        console.error('Error updating sync tracking for mini plan order creation:', trackingError);
      }
    ...
    if (planType === 'mini') {
      ...
      try {
        await SyncTracking.updateLatestTime(sellerId, 'planOrders');
        //(`🔄 Sync tracking updated: planOrders for mini plan activation ${targetPlanOrder._id}`);
      } catch (trackingError) {
        console.error('Error updating sync tracking for mini plan activation:', trackingError);
      }
    ...
    try {
      await SyncTracking.updateLatestTime(sellerId, 'planOrders');
      //(`🔄 Sync tracking updated: planOrders for plan activation ${targetPlanOrder._id}`);
    } catch (trackingError) {
      console.error('Error updating sync tracking for plan activation:', trackingError);
    }
