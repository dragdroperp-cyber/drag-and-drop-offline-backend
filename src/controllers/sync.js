...
const resolveEntity = async (Model, sellerId, localId, mongoId) => {
  ...
  if (localId) {
    const found = await Model.findOne({ localId, sellerId });
    if (found) return found;
  }
  return null;
};
  ...
  try {
    ...
    for (const item of items) {
      try {
        ...
        // Resolve Product
        let productId = null;
        let productLocalId = item.productId || item.productLocalId;
        // Check if item.productId is actually a localId or MongoId
        let productMongoId = item.productMongoId || (mongoose.isValidObjectId(item.productId) ? item.productId : null);
  ...
  try {
    ...
    for (const item of items) {
      try {
        ...
        if (!item.totalAmount || typeof item.totalAmount !== 'number') throw new Error('Order must have valid totalAmount');
        ...
        for (const orderItem of item.items) {
          ...
          let productLocalId = incomingProductLocalId || (incomingProductId && !mongoose.isValidObjectId(incomingProductId) ? incomingProductId : null);
          let productMongoId = mongoose.isValidObjectId(incomingProductMongoId) ? incomingProductMongoId : (mongoose.isValidObjectId(incomingProductId) ? incomingProductId : null);

          let resolvedDProductId = null;
          let dProductLocalId = incomingDProductLocalId || (incomingDProductId && !mongoose.isValidObjectId(incomingDProductId) ? incomingDProductId : null);
          let dProductMongoId = mongoose.isValidObjectId(incomingDProductMongoId) ? incomingDProductMongoId : (mongoose.isValidObjectId(incomingDProductId) ? incomingDProductId : null);
