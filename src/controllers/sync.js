...
const syncCustomers = async (req, res) => {
  try {
    ...
    // This ensures that new customers are created before their transactions reference them
    const sortedItems = [...items].sort((a, b) => {
      ...
      if (isATransaction && !isBTransaction) return 1; // Transaction comes after Customer
      if (!isATransaction && isBTransaction) return -1; // Customer comes before Transaction
      return 0;
    });
        ...
        if (isTransaction) {
          ...
          */
          ...
          if (existingCust) {
            ...
            existingCust.isDeleted = false; // Ensure it's not hidden if re-synced
    ...
    for (const item of items) {
      try {
        ...
        if (item.category) {
          ...
          if (mongoose.Types.ObjectId.isValid(item.categoryId)) {
            ...
            if (category) {
              ...
              console.warn(`Category not found for seller`);
            }
          } else {
        ...
        if (existing) {
          ...
          existing.isDeleted = false; // CRITICAL: Un-delete if found by name/id
    ...
    for (const item of items) {
      try {
        ...
        if (paymentMethod === 'split') {
          ...
          const creditAmount = typeof splitDetails.creditAmount === 'number' ? splitDetails.creditAmount : 0;
          const dueAmount = typeof splitDetails.dueAmount === 'number' ? splitDetails.dueAmount : 0;

          if (cashAmount < 0 || onlineAmount < 0 || creditAmount < 0 || dueAmount < 0) {
            throw new Error('Split payment amounts cannot be negative');
          }

          // Auto-infer type if missing or invalid
          const validSplitTypes = ['cash_online', 'online_due', 'cash_due', 'cash_online_due', 'credit_due', 'cash_credit', 'online_credit', 'cash_online_credit', 'cash_credit_due', 'online_credit_due', 'cash_online_credit_due', 'credit_only'];
          if (!splitDetails.type || !validSplitTypes.includes(splitDetails.type)) {
            ...
            else if (creditAmount > 0 && dueAmount > 0) splitDetails.type = 'credit_due';
            else if (creditAmount > 0) splitDetails.type = 'credit_only';
            else splitDetails.type = 'cash_online'; // Fallback
          }

          // Validate that amounts match the split type (simplified check as they are already all >= 0)

          // Validate that split amounts sum to totalAmount (within 0.1 tolerance)
          const splitTotal = cashAmount + onlineAmount + dueAmount + creditAmount;
          if (Math.abs(splitTotal - item.totalAmount) > 0.1) {
          ...
          if (orderItem.productId) {
            if (mongoose.Types.ObjectId.isValid(orderItem.productId)) {
              ...
              if (resolvedProduct) {
                ...
                console.warn('Order item has invalid and unresolveable productId, setting to null');
        ...
        if (item.customerId) {
          ...
          if (mongoose.Types.ObjectId.isValid(item.customerId)) {
            ...
            if (customerExists && customerExists.sellerId.toString() === sellerId.toString()) {
              ...
              console.warn('Customer not found or does not belong to seller');
            }
          } else {
        ...
        if (existing && existing.sellerId.toString() === sellerId.toString()) {
          ...
          if (paymentMethod === 'split' && item.splitPaymentDetails) {
            existing.splitPaymentDetails = {
              ...
              creditAmount: item.splitPaymentDetails.creditAmount || 0,
    ...
    for (const item of items) {
      try {
        ...
        if (existing && existing.sellerId.toString() === sellerId.toString()) {
          ...
          existing.cancelledAt = item.cancelledAt ? new Date(item.cancelledAt) : existing.cancelledAt;
          existing.cancelledReason = item.cancelledReason || existing.cancelledReason;
          existing.refundedAmount = item.refundedAmount !== undefined ? item.refundedAmount : existing.refundedAmount;

          existing.paymentMethod = item.paymentMethod || existing.paymentMethod;
          existing.amountPaid = item.amountPaid !== undefined ? item.amountPaid : existing.amountPaid;
          existing.balanceDue = item.balanceDue !== undefined ? item.balanceDue : existing.balanceDue;
          existing.paymentStatus = item.paymentStatus || existing.paymentStatus;
      ...
      try {
        ...
        if (item.customerId && !mongoose.Types.ObjectId.isValid(item.customerId)) {
          ...
          if (customer) {
            ...
            console.warn('Could not resolve customerId for transaction.');
            results.failed.push({ id: item.id, error: `Customer not found for ID: ${item.customerId}` });
            continue;
          }
    ...
    for (const item of sortedItems) {
      try {
        ...
        if (isTransaction) {
          ...
          // Reuse syncSupplierTransactions logic if possible, but here we inline for batch processing
          // Actually, let's just delegate one by one or implement inline
          // We will implement inline to handle the loop
          if (item.isDeleted === true) {
            let existing = (item._id && mongoose.Types.ObjectId.isValid(item._id)) ? await SupplierTransaction.findById(item._id) : null;
            if (!existing && item.id) {
          ...
          let existingTx = null;
          if (item._id && mongoose.Types.ObjectId.isValid(item._id)) {
            existingTx = await SupplierTransaction.findById(item._id);
            if (existingTx && existingTx.sellerId.toString() !== sellerId.toString()) existingTx = null;
          }
          if (!existingTx && item.id) {
            existingTx = await SupplierTransaction.findOne({ sellerId, localId: item.id });
          }

          let resolvedSupplierId = item.supplierId;
          if (item.supplierId && !mongoose.Types.ObjectId.isValid(item.supplierId)) {
            const supplier = await Supplier.findOne({ sellerId, localId: item.supplierId });
            if (supplier) {
              resolvedSupplierId = supplier._id;
            } else {
              results.failed.push({ id: item.id, error: `Supplier not found for ID: ${item.supplierId}` });
              continue;
            }
          }
          ...
          let resolvedOrderId = item.orderId;
          if (item.orderId && !mongoose.Types.ObjectId.isValid(item.orderId)) {
            const order = await VendorOrder.findOne({ sellerId, localId: item.orderId });
            resolvedOrderId = order ? order._id : null;
          }
          ...
          if (item.isDeleted === true) {
            let existing = null;
            if (item._id && mongoose.Types.ObjectId.isValid(item._id)) {
              existing = await Supplier.findById(item._id);
            }
            if (!existing && item.id) {
              existing = await Supplier.findOne({ sellerId, localId: item.id });
            }
          ...
          if (item._id && mongoose.Types.ObjectId.isValid(item._id)) {
            existingSup = await Supplier.findById(item._id);
            if (existingSup && existingSup.sellerId.toString() !== sellerId.toString()) {
              existingSup = null;
            }
          }
  ...
  try {
    ...
    for (const item of items) {
      try {
        ...
        let existing = null;
        if (item._id && mongoose.Types.ObjectId.isValid(item._id)) {
          existing = await SupplierTransaction.findById(item._id);
          if (existing && existing.sellerId.toString() !== sellerId.toString()) existing = null;
        }
        if (!existing && item.id) {
          existing = await SupplierTransaction.findOne({ sellerId, localId: item.id });
        ...
        if (item.supplierId && !mongoose.Types.ObjectId.isValid(item.supplierId)) {
          ...
          results.failed.push({ id: item.id, error: `Missing supplierId` });
          continue;
        }
