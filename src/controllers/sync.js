    ...
    for (const item of items) {
      try {
        ...
        if (item.isDeleted === true) {
          ...
          // If not found by _id, try by localId
          if (!existing && item.id) {
            //(`🗑️ Looking for customer by localId: ${item.id}`);
            existing = await Customer.findOne({ sellerId, localId: item.id });
          }

          // If not found by _id or localId, try to find by name or other identifiers
          if (!existing) {
            //(`🗑️ Customer not found by _id or localId, trying alternative lookup for item.id: ${item.id}`);
            if (item.name) {
              ...
              if (mobileNumber) {
                ...
                if (existing) {
                  //(`Found customer by name + mobile: ${existing._id}`);
                }
              }
            }
          }
        ...
        // If not found by _id, check for duplicate by localId then by name + mobileNumber
        if (!existing && item.id) {
          existing = await Customer.findOne({ sellerId, localId: item.id });
        }
    ...
    for (const item of items) {
      try {
        ...
        if (existing && existing.sellerId.toString() === sellerId.toString()) {
          ...
          if (diff !== 0 && existing.customerId && !item.dueAdded) {
          ...
          if (item.stockDeducted) {
            console.log(`[SYNC] ⏭️ Skipping batch adjustment for order ${item.id} - already deducted on frontend`);
          } else {
            console.log(`[SYNC] 🔄 Deducting from product batches for order ${item.id} (stockDeducted: ${!!item.stockDeducted})`);
          ...
          if (customerId && !item.dueAdded) {
            ...
            console.log(`[SYNC] 💰 Skipping customer balance update for order ${item.id} - already added on frontend`);
          }
