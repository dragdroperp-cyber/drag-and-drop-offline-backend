    ...
    for (const item of items) {
      try {
        ...
        if (item.isDeleted === true) {
          ...
          if (existing && existing.sellerId.toString() === sellerId.toString()) {
            // Delete from MongoDB
            await ProductCategory.findByIdAndDelete(item._id);
  ...
  try {
    ...
    for (const item of items) {
      try {
        ...
        // Check for duplicate by name + description (prevent duplicates)
        // Products with same name and description are considered duplicates
...
/**
 * Batch Sync - sync all data types at once
 */
const batchSync = async (req, res) => {
  try {
    const { categories, products, customers, orders, transactions, vendorOrders } = req.body;
    const results = {
      categories: { success: [], failed: [] },
      products: { success: [], failed: [] },
      customers: { success: [], failed: [] },
      orders: { success: [], failed: [] },
      transactions: { success: [], failed: [] },
      vendorOrders: { success: [], failed: [] }
    };

    // Sync in order: categories -> products -> customers -> orders -> transactions -> vendorOrders
    if (categories && categories.length > 0) {
      const catResult = await syncCategoriesInternal(req.sellerId, categories);
      results.categories = catResult;
    }

    if (products && products.length > 0) {
      const prodResult = await syncProductsInternal(req.sellerId, products);
      results.products = prodResult;
    }

    if (customers && customers.length > 0) {
      const custResult = await syncCustomersInternal(req.sellerId, customers);
      results.customers = custResult;
    }

    if (orders && orders.length > 0) {
      const orderResult = await syncOrdersInternal(req.sellerId, orders);
      results.orders = orderResult;
    }

    if (transactions && transactions.length > 0) {
      const transResult = await syncTransactionsInternal(req.sellerId, transactions);
      results.transactions = transResult;
    }

    if (vendorOrders && vendorOrders.length > 0) {
      const voResult = await syncVendorOrdersInternal(req.sellerId, vendorOrders);
      results.vendorOrders = voResult;
    }
    ...
    for (const item of items) {
      try {
        ...
        if (item.isDeleted === true) {
          ...
          if (existing && existing.sellerId.toString() === sellerId.toString()) {
            // Delete from MongoDB
            await Refund.findByIdAndDelete(item._id);
        ...
        if (existing && existing.sellerId.toString() === sellerId.toString()) {
          ...
          // Update product stock (increase stock for refunded items)
