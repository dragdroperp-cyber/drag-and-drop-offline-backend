...
const adjustProductStockForOrder = async (sellerId, orderItems) => {
  console.log(`🔄 [BATCH_REDUCTION] Starting batch stock reduction for ${orderItems.length} items`);
  if (!Array.isArray(orderItems) || orderItems.length === 0) {
    console.log(`🔄 [BATCH_REDUCTION] No items to process`);
    return;
  }
  ...
  for (const item of orderItems) {
    ...
    if (product.trackExpiry) {
      ...
      console.log(`🎯 Using FEFO for product ${product.name} (trackExpiry: ${product.trackExpiry})`);
    } else {
      // FIFO (First In, First Out): Sort by creation date ascending (oldest first)
      sortCriteria = { createdAt: 1 };
      console.log(`🎯 Using FIFO for product ${product.name} (trackExpiry: ${product.trackExpiry})`);
    }
      ...
      console.log(`🔍 Found ${batches.length} active batches for product ${item.productId}:`, batches.map(b => ({
        id: b._id.toString(),
        quantity: b.quantity,
        expiry: b.expiry
      })));

      console.log(`📦 Found ${batches.length} batches for product ${product.name}, needing ${quantity} units`);
      console.log(`📦 Processing order: FEFO=${batches.map(b => ({
      ...
      for (const batch of batches) {
        ...
        console.log(`💾 Saved batch ${batch._id} with new quantity: ${savedBatch.quantity}`);
        ...
        console.log(`📦 Deducted ${deductQuantity} from batch ${batch.batchNumber || batch._id} (${originalQuantity} → ${batch.quantity}) | Remaining needed: ${remainingQuantity}`);
      }
      ...
      if (remainingQuantity > 0) {
        ...
        console.log(`✅ Successfully deducted ${totalDeducted}/${quantity} units from ${deductionDetails.length} batches`);
        console.log(`📊 Deduction summary:`, deductionDetails);
        ...
        console.log(`🔍 Verification - Updated batches:`, updatedBatches.map(b => ({
          id: b._id.toString(),
  ...
  console.log(`✅ [BATCH_REDUCTION] Completed batch stock reduction for all ${orderItems.length} items`);
};
  ...
  try {
    ...
    for (const item of items) {
      try {
        ...
        if (item.isDeleted === true) {
          ...
          if (item._id) {
            console.log(`🗑️ Looking for customer by _id: ${item._id}`);
            existing = await Customer.findById(item._id);
            if (existing) {
              console.log(`🗑️ Found customer by _id: ${existing._id}, sellerId: ${existing.sellerId}`);
            } else {
              console.log(`🗑️ Customer not found by _id: ${item._id}`);
            }
          }

          // If not found by _id, try to find by frontend ID or other identifiers
          if (!existing) {
            console.log(`🗑️ Customer not found by _id, trying alternative lookup for item.id: ${item.id}`);
            ...
            if (item.name) {
              ...
              if (mobileNumber) {
                ...
                if (existing) {
                  console.log(`🗑️ Found customer by name + mobile: ${existing._id}`);
                }
              }
            }
          }
          ...
          if (existing && existing.sellerId.toString() === sellerId.toString()) {
            console.log(`🗑️ Deleting customer ${existing._id} (${existing.name}) from MongoDB`);
            // Hard delete from MongoDB - completely remove the document
            await Customer.findByIdAndDelete(existing._id);
            console.log(`✅ Permanently deleted customer ${existing._id} from MongoDB`);
            ...
            console.log(`⚠️ Customer not found in MongoDB or sellerId mismatch:`, {
              itemId: item.id,
              itemMongoId: item._id,
    ...
    if (results.success.length > 0 || deletionCount > 0) {
      try {
        ...
        console.log(`📊 Updated sync tracking for customers: ${customerCount} remaining (deleted ${deletionCount})`);
      } catch (trackingError) {
  ...
  try {
    ...
    if (results.success.length > 0 || deletionCount > 0) {
      try {
        ...
        console.log(`📊 Updated sync tracking for categories: ${categoryCount} remaining (deleted ${deletionCount})`);
      } catch (trackingError) {
  ...
  try {
      ...
      try {
        ...
        if (item.isDeleted === true) {
          ...
          if (existing && existing.sellerId.toString() === sellerId.toString()) {
            ...
            console.log(`Permanently deleted product ${existing._id} (${existing.name}) from MongoDB`);
            results.success.push({ id: item.id, _id: existing._id, action: 'deleted' });
            deletionCount++;
    ...
    if (results.success.length > 0 || deletionCount > 0) {
      try {
        ...
        console.log(`📊 Updated sync tracking for products: ${productCount} remaining (deleted ${deletionCount})`);
      } catch (trackingError) {
      ...
      try {
        ...
        if (item.isDeleted === true) {
          ...
          if (existing && existing.sellerId.toString() === sellerId.toString()) {
            ...
            console.log(`Permanently deleted product batch ${existing._id} from MongoDB`);
            results.success.push({ id: item.id, _id: existing._id, action: 'deleted' });
            deletionCount++;
          } else {
            // Item doesn't exist in backend, or _id doesn't match - treat as success
            console.log(`Product batch ${item.id} not found in MongoDB, treating deletion as success`);
            results.success.push({ id: item.id, _id: item._id || null, action: 'deleted' });
          }
          continue; // Skip to next item
        }
        ...
        // If not found by _id, try to find by unique combination
        if (!existing) {
        ...
        if (!product) {
          results.failed.push({
            id: item.id,
            error: `Product ${item.productId} not found or does not belong to seller`
          });
          continue;
        }
...
const syncOrders = async (req, res) => {
  try {
    ...
    console.log(`[SYNC] 📥 RECEIVED ORDER SYNC REQUEST:`, {
      sellerId,
    ...
    for (const item of items) {
      try {
        ...
        if (existing && existing.sellerId.toString() === sellerId.toString()) {
          ...
          console.log(`[SYNC] 💾 SAVING ORDER TO MONGODB:`, {
            id: item.id,
          ...
          console.log(`[SYNC] 🔄 Deducting from product batches for order ${item.id} with ${item.items?.length || 0} items`);
          await adjustProductStockForOrder(sellerId, item.items);
          console.log(`[SYNC] ✅ Batch deduction completed for order ${item.id}`);

          console.log(`[SYNC] 📄 Order created with invoice: ${saved.invoiceNumber}`);
    ...
    if (results.success.length > 0 || deletionCount > 0) {
      try {
        ...
        console.log(`📊 Updated sync tracking for orders: ${orderCount} remaining (deleted ${deletionCount})`);
      } catch (trackingError) {
  ...
  try {
    ...
    if (results.success.length > 0 || deletionCount > 0) {
      try {
        ...
        console.log(`📊 Updated sync tracking for vendorOrders: ${vendorOrderCount} remaining (deleted ${deletionCount})`);
      } catch (trackingError) {
  ...
  for (const item of items) {
    try {
      ...
      if (existing && existing.sellerId.toString() === sellerId.toString()) {
        ...
        // Always deduct from batches in backend (batch-based inventory system)
        // The stockDeducted flag was for old product-based system
        console.log(`[SYNC] Deducting from product batches for order ${item.id}`);
        await adjustProductStockForOrder(sellerId, item.items);
  ...
  try {
    ...
    if (results.success.length > 0 || deletionCount > 0) {
      try {
        ...
        console.log(`📊 Updated sync tracking for refunds: ${refundCount} remaining (deleted ${deletionCount})`);
      } catch (trackingError) {
