...
exports.createRefund = async (req, res) => {
  try {
        ...
        try {
          ...
          if (product) {
            ...
            if (recentBatch) {
              ...
              console.log(`📦 Refund: Added ${qty} units back to batch ${recentBatch.batchNumber || recentBatch._id} (${recentBatch.quantity})`);
            } else {
              console.warn(`No active batches found for product ${productId} during refund - quantity not restored`);
            }
          } else {
            console.warn(`Product ${productId} not found for refund stock update`);
          }
        } catch (error) {
    ...
    if (failedUpdates.length > 0) {
      ...
      console.error('Stock update failed after refund creation, refund record deleted:', failedUpdates);
    ...
    try {
      ...
      console.error('Error updating sync tracking for refund:', trackingError);
    }
