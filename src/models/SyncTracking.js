...
syncTrackingSchema.statics.updateLatestTime = async function(sellerId, dataType, recordCount = null) {
  try {
    ...
    console.log(`✅ Updated sync tracking for ${dataType}: ${now}`);
    return result;
  } catch (error) {
...
syncTrackingSchema.statics.getLatestUpdateTimes = async function(sellerId) {
  try {
    ...
    const dataTypes = ['customers', 'products', 'productBatches', 'categories', 'orders', 'planOrders', 'refunds', 'transactions', 'vendorOrders'];
    const result = {};
...
syncTrackingSchema.statics.initializeForSeller = async function(sellerId) {
  try {
    ...
    console.log(`✅ Initialized sync tracking for seller ${sellerId}`);
    return result;
  } catch (error) {
